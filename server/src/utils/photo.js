 ...
 */
const defaultPhoto = () => {
  return configs.host.files + configs.amazon.name.defaultAvatar.defaultPhoto
}
 ...
 */
const defaultAvatar = () => {
  return configs.host.files + configs.amazon.name.defaultAvatar
}
 ...
 */
const cover = (name) => {
  return name ? (`${configs.host.files}${configs.amazon.prefix.thumbnail}${name}`) : defaultPhoto()
}
...
const getPrefix = (name, typePrefix) => {
  return name ? `${configs.amazon.prefix[typePrefix]}${name}` : null
}
