...
class SidebarView extends React.Component {
  ...
  getScreenSize = () => {
    ...
    if (screenPx <= AppConst.screens['xs-max']) {
      return 'xs'
    }
    if ((screenPx >= AppConst.screens['sm-min'])
      && (screenPx <= AppConst.screens['sm-max'])) {
      return 'sm'
    }
    if ((screenPx >= AppConst.screens['md-min'])
      && (screenPx <= AppConst.screens['md-max'])) {
      return 'md'
    }
    if ((screenPx >= AppConst.screens['lg-min'])
      && (screenPx <= AppConst.screens['lg-max'])) {
      return 'lg'
    }
    if (screenPx >= AppConst.screens['xl-min']) {
      return 'xl'
    }
  }
  ...
  displaySidebarItem = () => {
    ...
    for (const item of submenus) {
      if (item.getElementsByClassName('ant-menu-submenu-title').length) {
  ...
  render() {
    ...
    return (
      ...
      >
        <img className="app-logo" src={ImageConst.logoGem} alt="" />
        <MenuView
          location={location}
...
SidebarView.__ANT_LAYOUT_SIDER = true
/* eslint no-underscore-dangle: ["error", { "allow": ["__ANT_LAYOUT_SIDER"] }] */
