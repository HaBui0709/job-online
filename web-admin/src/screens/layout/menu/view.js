...
const { SubMenu } = Menu
const HAS_CHILD_VIEW = ['users', 'recuiterments', 'cvs', 'businesses']

class MenuView extends React.Component {
  render() {
    const { mode, location, t } = this.props
          ...
          menus.map((item) => {
            if (!item.children || !item.children.length) {
              ...
              return (
                <SubMenu
                  key={item.id}
                  title={<span><Icon type={item.icon} theme="outlined" /><span>{t(item.name)}</span></span>}
                >
                  {
